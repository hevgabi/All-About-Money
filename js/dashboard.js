...
async function renderDashboard() {
  ...
  document.getElementById('stats-grid').innerHTML = `
    <div class="card card-sm"><div class="card-label">Today's Expenses</div><div class="card-value" style="color:var(--danger)">${formatMoney(todayExp)}</div></div>
    <div class="card card-sm"><div class="card-label">This Week Expenses</div><div class="card-value" style="color:var(--danger)">${formatMoney(weekExp)}</div></div>
    <div class="card card-sm"><div class="card-label">This Week Income</div><div class="card-value" style="color:var(--accent)">${formatMoney(weekInc)}</div></div>
    <div class="card card-sm"><div class="card-label">This Month Expenses</div><div class="card-value" style="color:var(--danger)">${formatMoney(monthExp)}</div></div>
  `;
  ...
  if (qtWallet) qtWallet.innerHTML = wallets.map(w => `<option value="${w.id}">${escapeHtml(w.name)}</option>`).join('');
